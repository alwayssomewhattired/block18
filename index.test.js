...
test("Concatenate these two arrays", () => {
  expect(concatOdds([1, 2, 3, 4], [5, 6, 7, 8, 9])).toStrictEqual([
    1, 3, 5, 7, 9,
  ]);
});

test("if cart is empty, display message", () => {
  expect(cartCheckout("false")).toBe(console.log("'sad violin sounds'"));
});

test("if cart is full, display message", () => {
  expect(cartCheckout("true")).toBe(
    console.log("'rambunctious saxophone solo")
  );
});

test("if yes, display message for sign-in page", () => {
  expect(nextPage("true")).toBe(console.log("welcome to sign-in"));
});
